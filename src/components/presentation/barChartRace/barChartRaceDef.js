function _1(md) {
  return md`
Custom Bar Chart Race
  `;
}

function _data(FileAttachment) {
  return FileAttachment('dataSource').csv({ typed: true });
}
...
function _ticker(width, margin, barSize, n, formatDate, keyframes) {
...
export default function define(runtime, observer) {
  ...
  const fileAttachments = new Map([
    [
      ...
      {
        url: new URL(
          './historicalPrices/ETH_LTC_BNB_LINK.csv',
          import.meta.url
        ),
        mimeType: 'text/csv',
        toString,
  ...
  main
    ...
    .define(
      'ticker',
      ['width', 'margin', 'barSize', 'n', 'formatDate', 'keyframes'],
      _ticker
    );
  main
